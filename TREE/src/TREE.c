...
char *string2HEX(char *t [4],char *s) {
    char i,chs;
    for(i = 0; s[i]; ++i) {
        chs = s[i] & 0X0F;
        t[3 * i + 2] = ' ';
...
char *int2hex(char *buffer [4],int intNum){
    sprintf (buffer, "%x", intNum);
    return (buffer);
}
							   ...
							   const void *sid)
{
 ...
 qapi_Status_t res = QAPI_OK;
    ...
    uint16   switchReg=0x01;
	uint16   alarmReg=0x05;
    uint16   coldReg=0x09;
    uint16   warmReg=0x0D;
	char*  buffer[4] = {}; 
	uint8 cnt = 0;

	if (topic == "$baidu/iot/shadow/TREE/update/snapshot")
	{
			cJSON *reported = NULL;
			cJSON *state = NULL;
		...
		if (msg != NULL)
		{
			...
			if (root == NULL)
			{
			MQTT_UART_DBG("msg pack into json error...");
			}
			else
			{
				reported = cJSON_GetObjectltem(root, "reported");
				switchState = cJSON_GetObjectltem(reported, "swich");
				state = cJSON_GetObjectltem(reported, "state");
				alarm = cJSON_GetObjectltem(reported,"alarm");
				cold = cJSON_GetObjectltem(state, "cold");
				warm = cJSON_GetObjectltem(state, "warm");
			}
			res = qapi_I2CM_Open(QAPI_I2CM_INSTANCE_004_E, &i2c_handle);    
    		res = qapi_I2CM_Power_On(i2c_handle);
			res = i2c_write_reg_block(slave_Address,switchReg,string2HEX(buffer,switchState),I2C_REG_16BIT);
			res = i2c_write_reg_block(slave_Address,alarmReg,int2hex(buffer,alarm),I2C_REG_16BIT);

			res = i2c_write_reg_block(slave_Address,coldReg,int2hex(buffer,cold),I2C_REG_16BIT);

			res = i2c_write_reg_block(slave_Address,warmReg,int2hex(buffer,warm),I2C_REG_16BIT);
    		res = qapi_I2CM_Close (i2c_handle);    
			MQTT_UART_DBG("write complete");
...
qapi_Status_t conn_mqtt()
{
	...
	uint8 will_qos = 1;
	char username_ptr[] = "bs4kuft/TREE";
	uint8 username_len = strlen(username_ptr);
	char password_ptr[] = "bs4kuft/TREE";
	uint8 password_len = strlen(password_ptr);
	char client_id[] = "BG96_test_demo";
...
{
	char topic[] = "$baidu/iot/shadow/TREE/update";
...
{
	char topic[] = "$baidu/iot/shadow/TREE/update/snapshot";
	int qos = 1;
